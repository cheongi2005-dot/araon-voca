...
function Home() {
  ...
  return (
    /* ✨ pt-4를 추가하여 안전 영역 아래로 로고를 더 밀어냈습니다 ✨ */
    <div className="min-h-screen bg-[#F8F9FA] dark:bg-[#0A0A0B] text-[#1A1A1A] dark:text-[#E0E0E0] p-8 pb-24 transition-colors duration-500 font-sans pt-[calc(env(safe-area-inset-top)+1rem)]">
        ...
        <nav className="flex-1 space-y-5 animate__animated animate__fadeInUp">
          {menus.map((m) => (
            <Link key={m.id} to={m.path} className="group block">
              <div className="flex items-center justify-between bg-white dark:bg-[#1E1E1E] p-6 rounded-[2.2rem] border border-transparent shadow-sm transition-all duration-300">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold tracking-tight">{m.sub}</p>
                  </div>
                </div>
        ...
        <footer className="mt-16 pt-8 border-t border-zinc-100 dark:border-zinc-800 space-y-6">
          <div className="bg-white dark:bg-[#1E1E1E] p-5 rounded-[2.2rem] shadow-sm">
            ...
            <select 
              ...
              className="w-full p-3.5 bg-[#F8F9FA] dark:bg-[#0A0A0B] rounded-2xl text-xs font-bold outline-none appearance-none text-center"
            >
          ...
          <button onClick={resetAllData} className="w-full p-4 bg-[#70011D] text-white rounded-[1.8rem] text-[10px] font-black uppercase tracking-[0.25em] shadow-lg active:scale-[0.98]">
            Reset Application Data
          </button>
          <p className="text-center text-[9px] font-medium text-zinc-300 dark:text-zinc-700 uppercase tracking-widest">© Araon School Coaching System</p>
        </footer>
      </div>
    </div>
  );
